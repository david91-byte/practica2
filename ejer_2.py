...
texto_num= """ NumPy is the fundamental package needed for scientific computing with Python.
...
lista_texto_palabras = texto_num.split(' ')
maximo = 0
palabra = ' '



for i in range(len(lista_texto_palabras)):
    if lista_texto_palabras.count(lista_texto_palabras[i]) > maximo:
        maximo = lista_texto_palabras.count(lista_texto_palabras[i])
        palabra = lista_texto_palabras[i]

print(f'La palabra que mas veces aparece es: \"{palabra}\" y se repite unas {maximo} veces')